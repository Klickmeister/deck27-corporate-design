...
const colorClasses = {
  "d27-blau": ".d27-blau",
  "d27-blau-dunkel": ".d27-blau-dunkel",
  "d27-indigo": ".d27-indigo",
  "d27-petrol": ".d27-petrol",
  "d27-gruen": ".d27-gruen",
  "d27-oliv": ".d27-oliv",
  "d27-grau": ".d27-grau",
  "d27-schwarz": ".d27-schwarz",
};
...
const homeLayout = (data) => {
  ...
  return `
    <section class="home ${additonalClasses} snap-in" data-js-observe id="${id}">
      <header>
        <h1 class="title" style="transition-delay:${randomDelays[0]}s">${title}</h1>
        <h2 class="subtitle" style="transition-delay:${randomDelays[1]}s">${subtitle}</h2>
      </header>
      ${videoHtml}
      ${teaserText}
      ${linksHtml}
...
const imageLayout = (data) => {
  const { id, image, title, text, text2, download } = data;
  const { linksHtml, colorClass, additonalClasses } = stripLayoutData(data);

  const downloadBlock = download ? `<a href="${download}" class="download"><span class="icon">download</span></a>` : '';

  const textblock1 = text ? `<p class="bu-title">${text} ${downloadBlock}</p>` : '';
  const textblock2 = text2 ? `<p class="bu-text">${text2}</p>` : '';

  return `
    <section class="image ${colorClass} ${additonalClasses} snap-in" data-js-observe id="${id}">
      <figure class="image-wrapper">
        <img src="${image}" alt="${title}">
        <figcaption>
          ${textblock1}
...
const textLayout = (data) => {
  const { id, marginalie, title, subtitle, teasertext, mengentext, download } = data;
  const { linksHtml, colorClass, additonalClasses } = stripLayoutData(data);

  const downloadBlock = download ? `<a href="${download}" class="download"><span class="icon">download</span></a>` : '';

  return `
    <section class="text ${colorClass} ${additonalClasses} snap-in" data-js-observe id="${id}">
     <div class="wrap">
      <p class="marginalie">${marginalie} ${downloadBlock}</p>
